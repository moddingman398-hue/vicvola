...
// --- VicVolA ADS externs ---
extern void MB_UpdateADS(float frametime);
extern float MB_ComputeADSFOV(float base);
// --- end VicVolA ADS externs ---
...
{
	// VicVolA: update ADS fraction each frame
	MB_UpdateADS(gpGlobals->frametime);

	// VicVolA: compute ADS-adjusted FOV
	CViewSetup modifiedView = view;
	modifiedView.fov = MB_ComputeADSFOV(view.fov);
