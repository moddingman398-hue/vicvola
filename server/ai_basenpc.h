...
public:
	...
	void SetLastHitGroup(int hg) { m_LastHitGroup = hg; }

protected:
	int m_LastHitGroup; // <- put it here, in CAI_BaseNPC
...
#endif // AI_BASENPC_H
// ============================================================================
// ADVANCED NPC DAMAGE RESPONSE SYSTEM FOR MAPBASE
// Includes: Gibbing, Stagger Animations, Weapon Dropping
// ============================================================================

// ---------- Add to ai_basenpc.h or your custom NPC header ----------

#ifndef NPC_DAMAGE_RESPONSE_H
#define NPC_DAMAGE_RESPONSE_H
...
public:
	...
	{
		const char* boneName;
		const char* gibModels[3];  // Multiple gib models per limb
		int gibCount;
		float gibVelocity;
		float boneScale;  // Scale to apply when gibbed (0 = hide)
	};

	static void ProcessDamage(CBaseAnimating* pVictim, const CTakeDamageInfo& info);
	static void CreateGibs(CBaseAnimating* pVictim, GibType_t gibType, const Vector& damagePos, const Vector& damageForce);
	static void ShrinkBone(CBaseAnimating* pVictim, const char* boneName, float scale);
	static GibType_t GetGibTypeFromHitgroup(int hitgroup);

private:
	static GibInfo_t s_GibInfo[GIB_MAX_TYPES];
};

//-----------------------------------------------------------------------------
// Damage Response System
//-----------------------------------------------------------------------------
class CNPCDamageResponse
{
public:
	enum StaggerType_t
	{
		...
		STAGGER_LEG_LEFT,
		STAGGER_LEG_RIGHT,
		STAGGER_SHOULDER_LEFT,
		STAGGER_SHOULDER_RIGHT,
		STAGGER_CHEST,
		STAGGER_HEAD_FLINCH,
		STAGGER_STUMBLE_FORWARD,
		STAGGER_STUMBLE_BACKWARD
	};
	...
	{
		const char* boneName;
		const char* gibModels[3];
		int gibCount;
		float gibVelocity;
		float boneScale;
	};

	struct StaggerInfo_t
	{
		const char* animationName;
		float duration;
		float movementSpeed;
		bool dropsWeapon;
		float weaponDropChance;
	};

	// Main processing functions
	static void ProcessDamageResponse(CAI_BaseNPC* pNPC, const CTakeDamageInfo& info, trace_t* ptr = nullptr);
	static void ProcessGibbing(CAI_BaseNPC* pNPC, const CTakeDamageInfo& info);
	static void ProcessStagger(CAI_BaseNPC* pNPC, const CTakeDamageInfo& info, int hitgroup);
	static void ProcessWeaponDrop(CAI_BaseNPC* pNPC, const CTakeDamageInfo& info, int hitgroup);
	...
	// Stagger functions
	static StaggerType_t GetStaggerTypeFromHitgroup(int hitgroup, const Vector& damageForce);
	static void PlayStaggerAnimation(CAI_BaseNPC* pNPC, StaggerType_t staggerType);

	// Weapon drop functions
	static void DropWeapon(CAI_BaseNPC* pNPC, bool temporary = true);
	static void CreateWeaponPickupBehavior(CAI_BaseNPC* pNPC);

private:
	static GibInfo_t s_GibInfo[GIB_MAX_TYPES];
	static StaggerInfo_t s_StaggerInfo[9]; // Matches StaggerType_t count
};
#endif