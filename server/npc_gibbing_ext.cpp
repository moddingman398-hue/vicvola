...
void MB_ApplyLimbDamageGibbing(CBaseEntity* victim, const CTakeDamageInfo& info, int hitgroup){
    CAI_BaseNPC* npc = dynamic_cast<CAI_BaseNPC*>(victim); if(!npc) return;
    const float dmg = info.GetDamage();
    switch(hitgroup){
        case HITGROUP_HEAD:{
            bool helmet=false; // TODO: wire your helmet flag
            if(!helmet && dmg >= mb_head_gib_min_dmg.GetFloat()){ npc->SetHealth(0); npc->Event_Killed(info); return; }
            npc->SetCondition(COND_LIGHT_DAMAGE); npc->ScheduleFromName("SCHED_BIG_FLINCH");
        }break;
        case HITGROUP_LEFTLEG: case HITGROUP_RIGHTLEG:{
